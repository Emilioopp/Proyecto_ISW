  ...
  const handleCrearAsignatura = async (e) => {
    ...
    try {
      ...
      if (response.data.status === 'Success') {
        ...
        showErrorAlert('Error', response.data.message || 'No se pudo crear la asignatura');
      }
    } catch (error) {
      showErrorAlert('Error', error.response?.data?.message || 'Error al crear la asignatura');
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-600 via-indigo-600 to-blue-600 p-4">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-2xl p-6">
          <h2 className="text-2xl font-bold text-gray-800 mb-4">Listado de Asignaturas</h2>
          {asignaturas.length === 0 ? (
            <p className="text-gray-500 text-center py-8">No hay asignaturas registradas</p>
          ) : (
                ...
                <tbody>
                  {asignaturas.map((asignatura) => (
                    <tr key={asignatura.id} className="border-b hover:bg-gray-50">
                      <td className="px-4 py-3 font-mono text-sm">{asignatura.codigo}</td>
