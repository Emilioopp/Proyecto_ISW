...
export async function crearAsignaturaService(data) {
    try {
        // Verifica si existe una asignatura con el mismo codigo
...
export async function getAsignaturaByIdService(id) {
    try {
        const asignatura = await asignaturaRepository.findOne({
            where: { id },
            relations: ["profesorAsignaturas", "profesorAsignaturas.profesor"]
        });
        ...
        const profesorAsignaturasPublic = (asignatura?.profesorAsignaturas || []).map(pa => {
            const prof = pa.profesor || null;
            const profesorPublic = prof ? (({ password, ...rest }) => rest)(prof) : null;
            return { ...pa, profesor: profesorPublic };
        });

        const profesoresAsignados = profesorAsignaturasPublic
            .map(pa => pa.profesor)
            .filter(Boolean);

        const asignaturaPublic = { ...asignatura, profesorAsignaturas: profesorAsignaturasPublic, profesoresAsignados };

        return [asignaturaPublic, null];
    } catch (error) {
...
export async function updateAsignaturaService(id, data) {
    try {
        ...
        // Verifica si el nuevo codigo ya existe en otra asignatura
...
export async function deleteAsignaturaService(id) {
    try {
        const asignatura = await asignaturaRepository.findOne({
            where: { id },
            relations: ["profesorAsignaturas", "estudianteAsignaturas", "evaluacionesPracticas"]
        });
        ...
        if (asignatura.profesorAsignaturas?.length > 0 || 
            asignatura.estudianteAsignaturas?.length > 0 || 
            asignatura.evaluacionesPracticas?.length > 0) {
        return [null, "No se puede eliminar la asignatura porque tiene profesores, estudiantes o evaluaciones asociadas"];
        }
