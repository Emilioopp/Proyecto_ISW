...
export async function crearProfesorService(data) {
    try {
        ...
        const existingByEmail = await userRepo.findOne({ where: { email: data.email } });
        if (existingByEmail) {
...
export async function asignarProfesorAsignaturaService(profesorId, asignaturaId) {
    try {
        // Verifica que el profesor existe
        const profesor = await userRepo.findOne({ where: { id: Number(profesorId) } });
        if (!profesor) {
        ...
        return [{
            ...
            asignatura: { id: asignatura.id, codigo: asignatura.codigo, nombre: asignatura.nombre },
        },
        null,];
    } catch (error) {
...
export async function desasignarProfesorAsignaturaService(profesorId, asignaturaId) {
    try {
...
export async function deleteProfesorService(id) {
    try {
        ...
        if (asignaciones.length > 0) {
            return [null, "No se puede eliminar el profesor porque tiene asignaturas asignadas. Desasigne primero."];
        }
