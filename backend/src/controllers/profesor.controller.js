  ...
  try {
    ...
    if (!profesorId || !asignaturaId) {
      return handleErrorClient(
        res,
        400,
        "profesorId y asignaturaId son requeridos"
      );
    }

    const [asignacion, error] = await asignarProfesorAsignaturaService(
      profesorId,
      asignaturaId
    );
    ...
    handleSuccess(
      res,
      201,
      "Profesor asignado a asignatura exitosamente",
      asignacion
    );
  } catch (error) {
...
export async function desasignarProfesorAsignatura(req, res) {
  try {
    ...
    const [result, error] = await desasignarProfesorAsignaturaService(
      profesorId,
      asignaturaId
    );
